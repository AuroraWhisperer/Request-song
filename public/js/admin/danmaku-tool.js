...
(function () {
  ...
  function init() {
    ...
    const updateCounter = () => { counter.textContent = `${message.value.length} / 1000`; };
    ...
    refreshState = async () => {
      ...
      try {
        ...
        accountState.textContent = state.loggedIn ? `UID ${state.accountUid || '-'}` : '未登录';
        roomState.textContent = state.roomId ? `房间 ${state.roomId}` : '未设置';
        target.textContent = requester.name
          ? `${requester.name}${requester.uid ? `（UID ${requester.uid}）` : ''}`
          : '暂无最近的随机点歌人';
        toggle.disabled = !requester.name;
        if (!requester.name) toggle.checked = false;
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        const response = await fetch('/api/bilibili/danmaku/send', {
          ...
          body: JSON.stringify({ message: text, mentionRequester: toggle.checked })
        });
        const payload = await response.json();
        if (!response.ok || !payload.ok) throw new Error(payload.error || '发送弹幕失败');
        message.value = '';
        updateCounter();
        setResult(`已发送：${payload.data?.message || text}`, 'good');
        toast(payload.data?.replyUname ? `弹幕已发送并 @${payload.data.replyUname}` : '弹幕已发送');
      } catch (error) {
